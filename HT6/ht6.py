...
class Sequence:
    ...
    # функция замены символов в self.seq для замены по 1 агрументу
    ...
    # функция замены символов в self.seq для замены по 2 агрументам
    def changer2(self, func):
        new_seq = ''
        for i in range(len(self.seq)):
            new_seq += func(self.seq[i], self.seq[i-1])
        self.seq = new_seq
...
def alpha(alphabet):
    def statistic(seq):
        stat = dict.fromkeys(alphabet, 0)
        for i in seq:
            stat[i] += 1
        return stat # возвращает статистику по использованию символов
    return statistic

def rules_of_change(i): # функция правил замены для 1 символа
    rules = {'A': 'U', 'U': 'A', 'G': 'C', 'C': 'G'}
    if i in rules:
        return rules[i]
    return i

def rules_of_change2(cur, prev): # функция правил замены для 2 символов
    rules = {'A': 'U', 'U': 'A', 'G': 'C', 'C': 'G'}
    if prev == 'U':
        return 'T'
    if cur in rules:
